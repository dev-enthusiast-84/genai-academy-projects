...
These are authored functional fixtures, not evidence of real-world accuracy. Run each case multiple times to assess model variability, and later add held-out graphs and request paraphrases without modifying the answer key to accommodate failures. The dataset is validated structurally; the application has not yet been implemented or evaluated against it.

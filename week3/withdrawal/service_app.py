...
def page(store, query, csrf='', consent=None, notice=''):
    ...
    if store.service == 'documents':
        ...
        if state == 'not_granted':
            form = f'''<article><h2>Find your next favorite class</h2>
            ...
            <p><button>Agree &amp; save fitness interests</button></p></form></article>'''
        elif state == 'active':
            form = f'''<article><h2>Fitness personalization is enabled</h2><p>You agreed to share your profile with Class Booking and Member Offers.</p>
            <form method="post" action="/delete-profile"><input type="hidden" name="csrf" value="{csrf}">
            <p><button>Delete my profile from Club Portal</button></p></form>
            <p>This removes your saved profile here. To withdraw sharing consent and remove linked information across apps,
            <a href="http://127.0.0.1:8501">open Recall</a>.</p>
            <form method="post" action="/consent"><input type="hidden" name="csrf" value="{csrf}"><input type="hidden" name="agree" value="on">
            <p><button>Retry approved sharing</button></p></form></article>'''
        else:
    ...
    <header><small>AVERY EXAMPLE · SYNTHETIC DEMO</small><h1>{heading}</h1><p>{description}</p></header>{brand_panel}
    {'<p role="status">' + esc(notice) + '</p>' if notice else ''}{form}<div class="count">{len(records)} visible items · <a href="/">Refresh</a></div>
    ...
    <footer>This application shows its own saved information. Open Recall to review and approve a withdrawal, then refresh this page to see the result.</footer></main></body></html>'''
        ...
        def do_GET(self):
            ...
            if store.service == 'documents':
                ...
                try:
                    ...
                    engine.close()
            self.respond(200, page(store, query, csrf, consent), 'text/html; charset=utf-8')
